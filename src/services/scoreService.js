...
const updateScoreForOneStudent = async (data, t ) => {
  ...
  const oldScore = await db.Score.findOne({where: {roundResultId: data.roundResultId, judgeId: data.judgeId}})
  ...
  await db.Score.update(
    ...
    { where: {roundResultId: data.roundResultId, judgeId: data.judgeId}, transaction: t }
  );
...
export default {
  ...
  checkScoreIsExists,updateScoreForOneStudent, updateScoreForOneStudent
};
